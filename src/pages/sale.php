...
<!-- <div class="container mt-5"> -->
    ...
    <!--<div class="topBack">-->
    <div class="filtro">
        <form method="post" action="index.php?pagina=venda&act=filtro" id="filtroGerVenda" class='topBack form-inline'>
            <table class='table-responsive' id="tabFiltro">
                <tr>
                    <td><input type="radio" name="tBusca" value="Pedido" <?= isset($tBusca) && $tBusca === 'Pedido' ? 'checked' : null; ?>/></td>
                    <td>Nº Pedido</td>
                ...
                <tr>
                    ...
                    <td><select name="vendedor" <?= empty($IDEmpresa) ? "disabled" : null; ?> >
                            <option value="0"></option>
                            <?php
                                if (isset($dVend)):
                                    foreach ($dVend as $k => $v):
                                        if ($v->LogON !== "SITE"): ?>
                                            <option data-id=<?= $k ?> value="<?= isset($v->LogON) ? $v->LogON : null ?>" <?= isset($vendedor) && isset($v->LogON) && $vendedor === $v->LogON ? "selected" : null ?> ><?= isset($v->LogON) ? $v->LogON : null ?></option>
                            <?php
                                        endif;
                                    endforeach;
                                endif; ?>
                    ...
                    <td class="pl-1"><input type="radio" name="vAtiva" value=1 <?= empty($vAtiva) || $vAtiva == 1 ? 'checked' : null; ?>/></td>
                    <td>Vendas Ativas</td>
                    <td><input type="radio" name="vAtiva" value=0 <?= empty($vAtiva) ? 'checked' : null; ?>/></td>
    ...
    <div class="container-fluid screen" >
        <div id="ocultarSel" style="float: right">Ocultar Coluna:
            <select name="colunas">
                <option value=""></option>
                <?php
                    for($x = 0; $x < count($cols) - 1 ; $x++):
                        if($cols[$x] !== "Pedido" && $cols[$x]
                            !== "CNPJeCPF" && $cols[$x] !== "IDEMPRESA"
                            && $cols[$x] !== "OBS"): ?><option value="<?= $x+1 ?>" >
                    <?= ( array_key_exists($x, $chTitles) ? $chTitles[$x] : $cols[$x] ) ?></option>
                <?php
                        endif;
                    endfor; ?>
            </select>
        ...
        <form action="#" id="altOrc" enctype="multipart/form-data" method="POST">
            <table id="tabGer" class="compact display nowrap" width="100%" >
                <thead>
                    <tr>
                        <th></th>
                            <?php if (isset($cols)):
                                ...
                                foreach ($cols as $col):
                                    ...
                                    <th <?= $align ?> ><?= (array_key_exists($x, $chTitles)?$chTitles[$x] : $col) ?></th>
                                    <?php $x++;
                                endforeach;
                            endif; ?>
