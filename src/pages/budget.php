...
<style>
    ...
    #boxes .window, #boxes2 .window2, #boxes3 .window3 {
        position: absolute;
        top: 70px;
        margin: auto;
        min-width: 1000px;
    ...
    #boxes .window, #boxes2 .window2, #boxes3 .window3,
    #boxes .window input, #boxes2 .window2 input, #boxes3 .window3 input {
        font-size: 13px;
    }

    #boxes #dialog, #boxes2 #dialog2, #boxes3 #dialog3 {
...
<script>
    ...
    function modalClient(companyId, cnpjCpf, table) {
        ...
        //$('.window').load('../modal/coletaModal.php?IDEmpresa=' + IDEmpresa + '&tabela=' + tabela + '&cpf_cnpj=' + cpf_cnpj);
        // $('#boxe_main').load('../paginas/coleta.php?IDEmpresa=' + IDEmpresa
        //         + '&tabela=' + tabela + '&cpf_cnpj=' + cpf_cnpj);
    }
    ...
    function btnAction(name) {
        switch(name) {
            ...
            case "Cancelar":
                ...
                break;
            case "MUDAR PARA PJ":
                //$("#dClient input[name=typeClient][table=PFisica]").attr("checked", true);
                //$("#dClient input[name=typeClient][table=PJuridica]").attr("checked", false);
                break;
            case "MUDAR PARA PF":
                //$("#dClient input[name=typeClient][table=PJuridica]").attr("checked", true);
                //$("#dClient input[name=typeClient][table=PFisica]").attr("checked", false);
                break;
            case "LIMPA BUSCA":
                $("#boxe_main input").val("");
                break;
    ...
    $(document).ready(function() {
        ...
        // $("#dClient [name=typeClient]").on("change", function() {
        //     // let IDCliente = $("#dClient [name=IDCliente]").val();
        //     if($("#dClient [name=CNPJeCPF]").val().length > 14) {
        //         $("#dClient [name=typeClient][table=PJuridica]").attr("checked",true);
        //         $("#dClient [name=typeClient][table=PFisica]").attr("checked",false);
        //     }
        // });
...
<section id="ancora" class="container-fluid" >
    ...
    <section class="row form-inline" id="info">
        ...
        <div id="dEntrega" class="col-lg-4 pb-1" >
            ...
            <select name="IDTransportadora" style="width: 250px; font-size:11px;font-weight: bolder">
                <?php
                    // if(isset($transp)) {
                    //     foreach($transp as $key => $value) {
                    //         if($value->IDEmpresa === $IDEmpresa) {
                    //             if($pedido->getTransportadora() != '') {
                    //                 $check = $pedido->getTransportadora()
                    //                     == $value->IDTransportadora ? 'selected' : null;
                    //             } else {
                    //                 $check = $value->RasSocial == 'SEM FRETE' ?
                    //                         'selected' : null;
                    //             }

                    //             echo '<option value="'.$value->IDTransportadora . '" '
                    //                     . $check . '>'
                    //                         . $value->RasSocial . '</option>';
                    //         }
                    //     }
                    // }
                    // $VencOrcamento = substr($pedido->VencOrcamento,0,10); ?>
