...
class Product extends Model implements Models
{
    public static $entity = "Produto_Venda";

    /** @var array */
    private $required = [];

    public function load(int $IdxProd, string $columns = "*")
    {
        $load = $this->read("SELECT {$columns} FROM " . self::$entity . " WHERE IdxProd=:IdxProd", "IdxProd={$IdxProd}");
    ...
    public function search(array $where)
    {
        ...
        $data = $this->read("SELECT * FROM " . self::$entity . " WHERE {$terms} ORDER BY Item ", $params);
    ...
    {
        if(!empty($this->IdxProd)) {
            $this->delete(self::$entity, "IdxProd=:IdxProd", "IdxProd={$this->IdxProd}");
        }

        if($this->fail()) {
            $this->message = "Não foi possível remover produto";
            return null;
        }
    ...
    {
        $lastData = $this->all(1, 0, "IdxProd", "IdxProd DESC");
        return ($lastData ? $lastData[0]->IdxProd + 1 : 1);
    }
    ...
    {
        $sql = (new CreateProductsTable())->up(self::$entity);
        return $this->createTable($sql);
    }

    public function dropThisTable()
    {
        $sql = (new CreateProductsTable())->down(self::$entity);
